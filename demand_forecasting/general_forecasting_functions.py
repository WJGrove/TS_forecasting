...
def upsample_groupedweeklyts_spark(df, date_col='ds', group_col='time_series_id'):
    ...
    expanded_dates = date_ranges.withColumn(
        "ds",
        F.explode(F.expr("sequence(first_date, last_date, interval 1 week)"))
    ).select(group_col, "ds")
...
def remove_data_prior_to_inactive_periods(df, value_col='y', date_col='ds', group_col='time_series_id', inactive_threshold=4):
    """
    ...
    global_max_possible_order_date = df.filter(F.col(date_col) <= F.lit(current_date)) \
                                       .agg(F.max(F.col(date_col)).alias('global_max_possible_order_date')) \
                                       .collect()[0]['global_max_possible_order_date']
    inactivity_threshold_date = global_max_possible_order_date - timedelta(weeks=inactive_threshold)

    # Define window specifications
    windowSpecForInactivity = Window.partitionBy(F.col(group_col)).orderBy(F.col(date_col)).rowsBetween(-(inactive_threshold - 1), 0)
    ...
    df = df.withColumn("max_rolling_period_sum_inactive", F.max(F.col("rolling_period_sum_inactive")).over(max_rolling_sum_window))

    # Create activity status flag fields
    df = df.withColumn(
        "is_obsolete",
        F.col("series_max_date") < F.lit(inactivity_threshold_date)
    )
    df = df.withColumn(
        "iswas_inactive",
        F.col("max_rolling_period_sum_inactive") >= inactive_threshold
    )
    df = df.withColumn(
        "never_inactive",
        F.col("max_rolling_period_sum_inactive") < inactive_threshold
    ...
    df_iswas_inactive = df_iswas_inactive.withColumn(
        ...
        F.max(
            F.when(
                F.col("rolling_period_sum_inactive") == F.col("max_rolling_period_sum_inactive"), 
                F.col(date_col)
                )
    ...
    df_iswas_inactive_filtered = df_iswas_inactive_filtered.drop("is_inactive", "max_inactive_date", "rolling_period_sum_inactive", "is_obsolete", "iswas_inactive", "never_inactive", "max_rolling_period_sum_inactive")
    df_never_inactive = df_never_inactive.drop("is_obsolete", "iswas_inactive", "never_inactive", "is_inactive", "rolling_period_sum_inactive", "max_rolling_period_sum_inactive")
    ...
    print(f"Final row count after combining 'never_inactive' and filtered 'iswas_inactive': {final_row_count}")
...
def remove_series_w_insuff_data(df, group_col, insufficient_data_threshold, value_col):
    ...
    series_length_df = df.groupBy(group_col).agg(F.count(F.col(value_col)).alias("series_length"))

    # Filter out series with series_length less than the specified threshold
    valid_series = series_length_df.filter(F.col("series_length") >= insufficient_data_threshold) \
                                   .select(group_col)
...
def fill_dim_nulls_bfill_ffill(df, group_col, date_col, cols_to_fill):
    ...
    """
    # Define window specifications
    windowSpec = Window.partitionBy(group_col).orderBy(F.col(date_col)).rowsBetween(Window.unboundedPreceding, Window.currentRow)
    windowSpecDesc = Window.partitionBy(group_col).orderBy(F.col(date_col).desc()).rowsBetween(Window.unboundedPreceding, Window.currentRow)
...
def spark_remove_outliers(df, cols_for_outlier_removal, group_col="time_series_id", outlier_threshold=3, absolute_threshold=1e5):
    """
    ...
    for col in cols_for_outlier_removal:
        ...
        df = df.withColumn(
            f"{col}_zscore", 
            F.when(F.col(f"{col}_stddev") > 1e-5,  # Ensure stddev is not zero or near-zero
                   (F.col(col) - F.col(f"{col}_mean")) / F.col(f"{col}_stddev"))
            .otherwise(F.lit(0))  # Set Z-score to 0 where stddev is zero or near-zero
        )
        ...
        df = df.withColumn(
            f"{col}_is_outlier", 
            F.when(
                ((F.abs(F.col(f"{col}_zscore")) > outlier_threshold) & F.col(f"{col}_zscore").isNotNull()) |
                (F.abs(F.col(col)) > absolute_threshold),  # Secondary absolute threshold
                F.lit(True)
...
def spark_pandas_interpolate_convert(df, group_col, numerical_cols, date_col, interpolation_method='linear', order=3):
    """
    ...
    def interpolate_group(group):
        ...
        for col in numerical_cols:
            ...
            if interpolation_method in ['spline', 'polynomial']:
                group[f"{col}_int"] = group[col].interpolate(method=interpolation_method, order=order).ffill().bfill()
            else:
                group[f"{col}_int"] = group[col].interpolate(method=interpolation_method).ffill().bfill()
        return group

    # Apply interpolation for each group
    interpolated_df = pandas_df.groupby(group_col).apply(interpolate_group).reset_index(drop=True)
...
def upsample_monthlyts_to_weeklyts_spark(df, date_col="ds"):
    ...
    expanded_df = all_weeks_df.join(df, all_weeks_df.all_weeks == df[date_col], "left").select("all_weeks", *original_cols_to_select)
...
def boxcox_multi_ts_sps(df, group_col='time_series_id', date_col='ds', value_col='y', lower_lambda_threshold=0.9, upper_lambda_threshold=1.1):
    """
    ...
    for group_key, group in transformed_df.groupby(group_col):
        ...
        if any(y <= 0):
            ...
            transformed_df.loc[group.index, transformed_column_name] = y  # No transformation
            continue
...
def plot_aggregated_data(spark_df, group_col1, value_col, group_col2=None):
    ...
    # Aggregate data
    if group_col2:
        aggregated_df = spark_df.groupBy(group_col1, group_col2).agg(F.sum(value_col).alias("sum"))
    ...
    if group_col2:
        ...
        pd_df.pivot(index=group_col1, columns=group_col2, values='sum').plot(kind='bar', ax=plt.gca())
    else:
