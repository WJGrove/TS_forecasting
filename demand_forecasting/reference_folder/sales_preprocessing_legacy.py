...
cols_for_interpolation = [
    col for col in numerical_cols if col not in [value_col]
]  # we'll interpolate y_clean instead 'y'

interpolation_method = "linear"  # 'polynomial' 'spline'
interpolation_order = (
    3  # only applies when interpolation_method is 'polynomial' or 'spline'.
)
seasonal_period = 52
...
inactive_threshold = (
    4  # Any series ending with this number of consecutive zeros is discarded.
)
...
df_weekly = df.groupBy("ds", "time_series_id").agg(
    ...
    (F.sum(F.col("gross_price_fc") * F.col("y")) / F.sum(F.col("y"))).alias(
        "gross_price_fc"
    ),
...
df_inact_rem = remove_data_prior_to_inactive_periods(
    df_dates_filled, value_col, date_col, group_col, inactive_threshold
)
print()
...
df_insuff_data_rem = remove_series_w_insuff_data(
    df_inact_rem, group_col, insufficient_data_threshold, value_col
)
...
null_counts = df_insuff_data_rem.agg(
    F.count(F.when(F.col("parent_company_fc").isNull(), 1)).alias(
        "nulls_in_parent_company_fc"
    ),
    F.count(F.when(F.col("soldto_id").isNull(), 1)).alias("nulls_in_soldto_id"),
    F.count(F.when(F.col("item_id_fc").isNull(), 1)).alias("nulls_in_item_id_fc"),
    F.count(F.when(F.col("gross_price_fc").isNull(), 1)).alias(
        "nulls_in_gross_price_fc"
    ),
    F.count(F.when(F.col("y").isNull(), 1)).alias("nulls_in_y"),
    F.count(F.when(F.col("y") == 0, 1)).alias("y_is_zero"),
)
...
cols_to_fill = [
    col for col in df_dates_filled.columns if col not in cols_excluded_from_filling
]


df_dims_filled = fill_dim_nulls_bfill_ffill(
    df_insuff_data_rem, group_col, date_col, cols_to_fill
)
...
df_out_rem = spark_remove_outliers(
    df_dims_filled,
...
interpolated_df = spark_pandas_interpolate_convert(
    df_out_rem,
    group_col,
    cols_for_interpolation,
    date_col,
...
null_counts = interpolated_df.agg(
    ...
    F.count(F.when(F.col("gross_price_fc_int").isNull(), 1)).alias(
        "nulls_in_gross_price_fc_int"
    ),
    F.count(F.when(F.col("y").isNull(), 1)).alias("nulls_in_y"),
    # F.count(F.when(F.col('y') == 0, 1)).alias('y_is_zero'),
...
series_lengths_all = interpolated_df.groupBy("time_series_id").agg(
    F.count("ds").alias("series_length")
)
...
interpolated_df = interpolated_df.withColumn(
    "is_short_series",
    F.when(interpolated_df["series_length"] < short_series_threshold, True).otherwise(
        False
    ),
)
...
short_series_lengths = series_lengths_all.filter(
    series_lengths_all["series_length"] < short_series_threshold
)
...
total_y_short_series = short_series_df.agg(
    F.sum("y_clean").alias("sum_y_short")
).collect()[0]["sum_y_short"]
...
min_date_short_series = short_series_df.agg(F.min("ds").alias("min_date")).collect()[0][
    "min_date"
]
filtered_interpolated_df = interpolated_df.filter(
    interpolated_df["ds"] >= min_date_short_series
)
sum_y_filtered_interpolated_df = filtered_interpolated_df.agg(
    F.sum("y_clean").alias("sum_y_filtered")
).collect()[0]["sum_y_filtered"]
...
if percent_of_total_vol < 5:
    ...
    print(
        ...
        f"Is the forecast method used for short series appropriate for this volume?"
        + Style.RESET_ALL
    )
    ...
    print(
        Back.RED
        + f"The percent of the last {short_series_threshold} weeks' volume represented by short series is "
        f"{percent_of_total_vol:.2f}%. Confirm that the forecast method used for short series is appropriate for this volume."
        + Style.RESET_ALL
    )
...
series_lengths = (
    vis_trim_df.groupBy("time_series_id").agg(F.count("ds").alias("length")).cache()
)
...
series_volumes = (
    vis_trim_df.groupBy("time_series_id")
...
last_52_weeks_df = vis_trim_df.filter(
    (F.col("ds") < F.lit(last_sunday))
...
bottle_type_df = (
    last_52_weeks_df.groupBy("bottle_type")
    .agg(F.sum("y_clean_int").alias("total_volume"))
...
vis_trim_df_pd = (
    vis_trim_df.groupBy(["ds", "parent_company_fc", "bottle_type"])
...
last_52_weeks_start_date = last_sunday - pd.Timedelta(
    weeks=52
)  # last_sunday was defined previously
...
for bottle_type in vis_trim_df_pd["bottle_type"].unique():
    ...
    bottle_df = vis_trim_df_pd[
        (vis_trim_df_pd["bottle_type"] == bottle_type)
    ...
    top_companies = (
        bottle_df_52_weeks.groupby("parent_company_fc")["y_clean_int"]
        .sum()
        .nlargest(10)
        .index
    )
    top_bottle_df = bottle_df_52_weeks[
        bottle_df_52_weeks["parent_company_fc"].isin(top_companies)
    ]

    # Plot bar chart of total volume for the top ten companies (within the last 52 weeks)
    plt.figure(figsize=(10, 5))
    top_bottle_df.groupby("parent_company_fc")["y_clean_int"].sum().sort_values(
        ascending=False
    ...
    top_five_companies = (
        bottle_df.groupby("parent_company_fc")["y_clean_int"].sum().nlargest(5).index
    )
    top_five_bottle_df = bottle_df[
        bottle_df["parent_company_fc"].isin(top_five_companies)
    ]
...
line_plt_agg_df = vis_trim_df.groupBy(
    "bottle_type", F.year("ds").alias("year"), F.dayofyear("ds").alias("day_of_year")
...
for bottle_class in bottle_classes:
    ...
    pivot_df = class_df.pivot(
        index="day_of_year", columns="year", values="sum_y_clean_int"
    )

    # Ensure pivot_df covers the full range of days for each year
    x = pivot_df.columns.values
    y = np.arange(1, 367)  # Days of the year
    pivot_df = pivot_df.reindex(columns=x, index=y, fill_value=np.nan).fillna(
        method="ffill", axis=0
    )
...
transformed_df = boxcox_multi_ts_sps(
    interpolated_df, group_col, value_col="y_clean_int", date_col="ds"
)
...
transformed_df = transformed_df.withColumn(
    "series_median", F.expr("percentile_approx(y_clean_int, 0.5)").over(windowSpec)
)
...
null_counts = transformed_df.agg(
    ...
    F.count(F.when(F.col("y_clean").isNull(), 1)).alias("nulls_in_y_clean"),
    F.count(F.when(F.col("y_clean_int").isNull(), 1)).alias("nulls_in_y_clean_int"),
    F.count(F.when(F.col("y_clean_int_transformed").isNull(), 1)).alias(
        "nulls_in_y_clean_int_transformed"
    ),
...
transformed_df_with_update_date = transformed_df.withColumn(
    "table_update_datetime", F.lit(table_update_date)
)

transformed_df_with_update_date.createOrReplaceGlobalTempView(
    "transformed_df_with_update_date"
)
